...
conversations = [create_conversation(item) for item in data]
...
training_args = SFTConfig(
        ...
        output_dir = os.path.join(finetuning_output_dir, "gemma-3-4b-it-arxiv-instruct-qlora"), 
    )
...
messages = [{
    "role": "user",
    "content": [{"type" : "text", 
                 "text" : "Compare ARIMA with other models for time series forecasting",}]
}]
...
trainer.create_model_card({'tags': ['sft', 'gemma-3', 'arxiv-instruct']})

# model.push_to_hub("mieng155/gemma-3-4b-it-arxiv-instruct-qlora", token=hf_token) # Online saving
# tokenizer.push_to_hub("mieng155/gemma-3-4b-it-arxiv-instruct-qlora", token=hf_token) # Online saving
...
if load_saved_adapters:
    model, tokenizer = FastModel.from_pretrained(
        model_name = "mieng155/gemma-3-4b-it-arxiv-instruct-qlora", 
        max_seq_length = 2048,
        load_in_4bit = True,
    )
